...
class TaskQueueWorker:
    ...
    async def _process_task(self, task: TaskEnvelope) -> TaskResult:
        ...
        if provider is None:
            ...
            return TaskResult(
                ...
                error=f"No provider for task type: {task.task_type}"
            )
    ...
    async def _fallback_handler(self, task: TaskEnvelope) -> TaskResult:
        ...
        """
        self.logger.warning(
            f"Circuit breaker open for task {task.task_id}, "
            "signaling fallback to local processing"
        )
