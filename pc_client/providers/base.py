...
class BaseProvider(ABC):
    ...
    async def process_task(self, task: TaskEnvelope) -> TaskResult:
        ...
        """
        if not self._initialized:
            return TaskResult(task_id=task.task_id, status=TaskStatus.FAILED, error="Provider not initialized")
        ...
        try:
            ...
            self.logger.info(f"Task {task.task_id} completed in {processing_time_ms:.2f}ms")

            # Record metrics
            _import_metrics()
            if _task_duration_seconds:
                provider_name = self.name.replace("Provider", "")
                _task_duration_seconds.labels(provider=provider_name, task_type=task.task_type.value).observe(
            ...
            return TaskResult(
                task_id=task.task_id, status=TaskStatus.FAILED, error=str(e), processing_time_ms=processing_time_ms
            )
